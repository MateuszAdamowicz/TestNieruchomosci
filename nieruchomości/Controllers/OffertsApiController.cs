...
namespace nieruchomości.Controllers
{
    public class OffertsApiController : ApiController
    {
        ...
        {
            List<House> houseData = _genericRepository.GetSet<House>().Where(x => x.Visible && x.Deleted == false).OrderByDescending(x => x.CreatedAt).ToList();
        ...
        {
            List<Flat> flatData = _genericRepository.GetSet<Flat>().Where(x => x.Visible && x.Deleted == false).OrderByDescending(x => x.CreatedAt).ToList();
        ...
        {
            List<Land> landData = _genericRepository.GetSet<Land>().Where(x => x.Visible && x.Deleted == false).OrderByDescending(x => x.CreatedAt).ToList();
